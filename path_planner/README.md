...
| `astar_cost_weight` | 2.5 | Terrain cost penalty multiplier (↑=prefer open space) |

### [v7] Costmap Inflation

A* inflates obstacles by the robot's inscribed radius before planning. This ensures global paths maintain enough clearance for the full robot body to pass.

**2-Zone Inflation:**
- **Inner zone** (0 ~ inscribed radius 0.425m = 5 cells): **Lethal** — A* cannot plan through here
- **Outer zone** (inscribed ~ inflation radius 0.525m = 6 cells): **Decaying cost** — A* avoids but can pass if needed

| Parameter | Default | Description |
|-----------|---------|-------------|
| `inflation_radius` | -1 (auto) | Obstacle inflation radius [m]. -1=auto (inscribed+margin=0.525m) |
| `clearance_margin` | 0.1 | Extra buffer around footprint [m] |

**Tuning:**
- Robot clips obstacles on turns → increase `inflation_radius` (e.g. 0.7m)
- Robot can't find paths through wide corridors → decrease `inflation_radius` (e.g. 0.4m)
- Runtime: `ros2 param set /path_planner_node inflation_radius 0.6`
